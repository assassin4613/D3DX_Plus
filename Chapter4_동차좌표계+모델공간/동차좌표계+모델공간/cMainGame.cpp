...
void cMainGame::Update()
{
	...
	if( SUCCEEDED( m_pDevice->BeginScene() ))
	{
		...
		m_pDevice->SetStreamSource( 0, m_pVBRHW, 0,sizeof(SVertexRHW));
		m_pDevice->SetFVF(SVertexRHW::CUSTOM_FVF);
...
void cMainGame::MakeVB()
{
	...
	SVertexRHW	sVertex[3] = 
	{
		{ 100.f, 300.f, 0.f, 1.f, 0xffff0000 },
		{ 300.f, 300.f, 0.f, 1.f,  0xff00ff00 },
		{ 200.f, 200.f, 0.f, 1.f,  0xff0000ff },
	};

	m_pDevice->CreateVertexBuffer(
				sizeof(SVertexRHW) * 3,	//전체 버텍스 버퍼의 크기
				0,
				SVertexRHW::CUSTOM_FVF,
				D3DPOOL_MANAGED,
				&m_pVBRHW, 0);
	...
	m_pVBRHW->Lock(
		0,	// 어디에서부터 락을걸꺼냐
		sizeof(SVertexRHW) * 3,	//어디까지 락을걸거냐
		&pData,
		0 );

	memcpy(pData, sVertex, sizeof(SVertexRHW) * 3 );
	m_pVBRHW->Unlock();
	...
	memcpy(pData2, sVertex2, sizeof(SVertex) * 3 );
	m_pVB->Unlock();
}
